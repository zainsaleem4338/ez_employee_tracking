class Team < ActiveRecord::Base
  EMPLOYEE_TYPE = ['Team Member', 'Team Leader'].freeze
  ...
  def create_team(team_lead_id, employee_ids)
    ...
    begin
      self.transaction do
        self.save!
        self.employee_teams.create!(employee_id: team_lead_id, employee_type: EMPLOYEE_TYPE[1])
        if team_members_ids.empty?
          self.errors.add(:base, 'Team members name required')
          return false
        end
        team_members_ids.each do |team_member|
          self.employee_teams.create!(employee_id: team_member, employee_type: EMPLOYEE_TYPE[0])
        end
        return self
      end
  ...
  def update_team(team_lead_id, employee_ids)
    ...
    begin
      ...
      team_lead = team_employees.find_by(employee_type: EMPLOYEE_TYPE[1])
      team_members = team_employees.where(employee_type: EMPLOYEE_TYPE[0]).pluck(:employee_id)
      ...
      self.transaction do
        self.save!
        team_lead_query.first_or_create!(employee_type: EMPLOYEE_TYPE[1])
        previous_team_member_ids.each do |member_id|
          team_employees.find_by(employee_id: member_id).delete
        end
        team_members_ids.each do |team_member|
          team_employees.where(employee_id: team_member).first_or_create!(employee_type: EMPLOYEE_TYPE[0])
        end
        return self
      end
    rescue ActiveRecord::RecordInvalid => error
  ...
  def add_errors(error)
    if error.record.is_a? EmployeeTeam
      if error.record.employee_id.nil? && error.record.employee_type == EMPLOYEE_TYPE[1]
        self.errors.add(:base, 'Team leader name is required')
      end
      if error.record.employee_id.nil? && error.record.employee_type == EMPLOYEE_TYPE[0]
